...
class Triangle(Shape):
    def __init__(self, figure_name, side_a, side_b, side_c):
        try:
            ...
            if float(side_a) + float(side_b) <= float(side_c) or float(side_b) + float(side_c) <= float(side_a) or float(side_a) + float(side_c) <= float(side_b):  # po co tyle razy konwertować?
                raise Exception("Triangle condition has not been met.") # nie wolno rzucać Exception - typ nic nie mówi
